...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const context = await getOperationsApiContext('delivery-accept', 60, 600, 'deliverer'); if ('response' in context) return context.response;
  const audit = getAuditRequestContext(request);
  const { error } = await context.adminClient.rpc('deliverer_accept_order_delivery', { p_actor_id: context.actor.id, p_delivery_id: id.data, p_request_id: audit.requestId, p_client_ip: audit.clientIp, p_user_agent: audit.userAgent });
