...
import { cancellationRequestSchema, entityUuidSchema } from '@/lib/orders/validation';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const context = await getOperationsApiContext('order-cancel-request', 10, 600, 'customer'); if ('response' in context) return context.response;
  ...
  const parsed = cancellationRequestSchema.safeParse(body.value); if (!parsed.success) return NextResponse.json({ error: parsed.error.issues[0]?.message ?? 'Enter a cancellation reason.' }, { status: 400, headers: OPERATIONS_PRIVATE_HEADERS });
  const audit = getAuditRequestContext(request);
  const { error } = await context.adminClient.rpc('customer_request_order_cancellation', { p_actor_id: context.actor.id, p_order_id: id.data, p_reason: parsed.data.reason, p_request_id: audit.requestId, p_client_ip: audit.clientIp, p_user_agent: audit.userAgent });
