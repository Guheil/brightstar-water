...
import { entityUuidSchema, cancellationResolutionSchema } from '@/lib/orders/validation';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsed = cancellationResolutionSchema.safeParse(body.value); if (!parsed.success) return NextResponse.json({ error: parsed.error.issues[0]?.message ?? 'Invalid request.' }, { status: 400, headers: OPERATIONS_PRIVATE_HEADERS });
  ...
  const context = await getOperationsApiContext('admin-cancellation-review', 30, 600, 'admin'); if ('response' in context) return context.response;
  const audit = getAuditRequestContext(request);
  const extra = { p_decision: parsed.data.decision, p_note: parsed.data.note ?? null };
  const { error } = await context.adminClient.rpc('admin_resolve_order_cancellation', { p_actor_id: context.actor.id, p_order_id: id.data, ...extra, p_request_id: audit.requestId, p_client_ip: audit.clientIp, p_user_agent: audit.userAgent });
