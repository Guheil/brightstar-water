...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const context = await getOperationsApiContext('admin-order-confirm', 30, 600, 'admin'); if ('response' in context) return context.response;
  const audit = getAuditRequestContext(request);
  const extra = {};
  const { error } = await context.adminClient.rpc('admin_confirm_order', { p_actor_id: context.actor.id, p_order_id: id.data, ...extra, p_request_id: audit.requestId, p_client_ip: audit.clientIp, p_user_agent: audit.userAgent });
