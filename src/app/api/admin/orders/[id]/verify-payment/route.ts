...
import { entityUuidSchema, paymentVerificationSchema } from '@/lib/orders/validation';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsed = paymentVerificationSchema.safeParse(body.value); if (!parsed.success) return NextResponse.json({ error: parsed.error.issues[0]?.message ?? 'Invalid request.' }, { status: 400, headers: OPERATIONS_PRIVATE_HEADERS });
  ...
  const context = await getOperationsApiContext('admin-payment-verify', 30, 600, 'admin'); if ('response' in context) return context.response;
  const audit = getAuditRequestContext(request);
  const extra = { p_reference: parsed.data.reference ?? null };
  const { error } = await context.adminClient.rpc('admin_verify_order_payment', { p_actor_id: context.actor.id, p_order_id: id.data, ...extra, p_request_id: audit.requestId, p_client_ip: audit.clientIp, p_user_agent: audit.userAgent });
