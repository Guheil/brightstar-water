...
/**
 * Catalog update kept immutable so Customer and Admin selectors receive the same state change.
 */
 ...
 */
export function deleteProductState(productId: string): CommandResult<Product> {
  const state = useAppStore.getState();
  const product = state.catalog.products.find((item) => item.id === productId);
  if (!product) return commandFailure('not_found', 'Product not found.');
...
/**
 * Adds or replaces a product in application state. New products receive a
