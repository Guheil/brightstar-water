-- Correct evidence path regexes to avoid backslash ambiguity.
alter table public.payments drop constraint if exists payments_proof_path_check;
alter table public.payments add constraint payments_proof_path_check check (proof_path is null or proof_path ~ '^payments/[0-9a-f-]{36}/[0-9a-f-]{36}[.]webp$');
alter table public.deliveries drop constraint if exists deliveries_completion_proof_path_check;
alter table public.deliveries add constraint deliveries_completion_proof_path_check check (completion_proof_path is null or completion_proof_path ~ '^deliveries/[0-9a-f-]{36}/[0-9a-f-]{36}[.]webp$');
